...
#include "imx6_demonstartor_constants.h"
...
static int frameBufferFile = 0;
static struct fb_var_screeninfo vinfo;
static struct fb_fix_screeninfo finfo;
static long int screenSize = 0;
static char* pFrameBuffer = 0;
static GILBoolean fbWasInited = GIL_FALSE;

static GILBoolean loadFrameBufferDevice(const char* checkDevice)
{
    // Open the file for reading and writing
    frameBufferFile = open(checkDevice, O_RDONLY);
    if (frameBufferFile == -1) {
        perror("Error: cannot open framebuffer device");
        return GIL_FALSE;
    }
    printf("Will check FB: %s\n", checkDevice);

    // Get fixed screen information
    if (ioctl(frameBufferFile, FBIOGET_FSCREENINFO, &finfo) == -1) {
        perror("Error reading fixed information");
        return GIL_FALSE;
    }

    // Get variable screen information
    if (ioctl(frameBufferFile, FBIOGET_VSCREENINFO, &vinfo) == -1) {
    ...
    pFrameBuffer = (char *)mmap(0, screenSize, PROT_READ, MAP_SHARED, frameBufferFile, 0);
...
// static void printFrameBufferDevice(int32_t x1, int32_t y1, int32_t x2, int32_t y2)
// {
...
//     {
//         printf("\t");
//         for (int32_t curX = x1; curX < x2; ++curX)
//         {
//             printf("%d\t", curX);
//         }
//         printf("\n");
//         for (int32_t curY = y1; curY < y2; ++curY)
//         {
//             printf("%d\t", curY);
//             for (int32_t curX = x1; curX < x2; ++curX)
//             {
//                 printf("%d\t", *((uint16_t*)pFrameBuffer + finfo.line_length/2 * curY + curX));
//             }
//             printf("\n");
//         }
//         break;
...
static uint16_t getPixelFromFB(int32_t x, int32_t y)
{
    return *((uint16_t*)pFrameBuffer + finfo.line_length/2 * y + x);
}

static GILBoolean checkPoints(const gil_check_texture* points)
{
    GILBoolean res = GIL_FALSE;
    if (points != 0)
    {
        uint16_t point1 = getPixelFromFB(points->point[0].x, points->point[0].y);
        uint16_t point2 = getPixelFromFB(points->point[1].x, points->point[1].y);
        uint16_t point3 = getPixelFromFB(points->point[2].x, points->point[2].y);
        res = point1 == points->point[0].value && point2 == points->point[1].value &&
              point3 == points->point[2].value;
    }
...
static NativeDisplayType getNativeDisplay(int fbNumber)
{
    NativeDisplayType dsp = fbGetDisplayByIndex(fbNumber);
    if(0 == dsp)
    {
        fprintf(stderr, "CreateNativeDisplay failed\n");
    }
    else
    {
        printf("FB%d for output was opened successfully.\n", fbNumber);
    }
    return dsp;
}
...
{
    NativeWindowType wnd = fbCreateWindow(dsp, 0, 0, width, height);
    if(0 == wnd)
    {
        fprintf(stderr, "CreateNativeWindow failed\n");
    }
    return wnd;
}
...
{
    // EGLDisplay
    m_display.native = getNativeDisplay(config->renderDevice);
    ...
    fbWasInited = loadFrameBufferDevice(config->checkDevice);
}
...
GILBoolean gilVerify(GILContext ctx, int32_t x1, int32_t y1, int32_t u1, int32_t v1, int32_t x2, int32_t y2, int32_t u2, int32_t v2)
{
    ...
    {
        /**
         * Currently the etalon values are hard coded.
         * They are taken for Demonstrator project and is hard linked to this project.
         * TODO: get these etalon values from OpenGL frame buffer.
         */
        switch (x1)
        {
        case AIRBAG_COORD:
        {
            res = checkPoints(&m_airbag);
            break;
        }
        case ABS_COORD:
        {
            res = checkPoints(&m_abs);
            break;
        }
        case FOG_COORD:
        {
            res = checkPoints(&m_fog);
            break;
        }
        case ESP_COORD:
        {
            res = checkPoints(&m_esp);
            break;
        }
        case PARKING_COORD:
        {
            res = checkPoints(&m_parking);
            break;
        }
        case PARKING_BRAKE_COORD:
        {
            res = checkPoints(&m_parkingBreak);
            break;
        }
        case LOW_BEAM_COORD:
        {
            res = checkPoints(&m_lowBeam);
            break;
        }
        case HIGH_BEAM_COORD:
        {
            res = checkPoints(&m_highBeam);
            // printFrameBufferDevice(x1, y1, x2, y2);
            break;
        }
        default:
        {
            printf("Unknown texture! X1: %d\n", x1);
            break;
        }
        }
    }
