...
TEST(InputStreamTest, TestBytesToReadWithCorruptedData)
{
    ...
    corrupter->setError(PSC_NO_ERROR);
...
TEST(InputStreamTest, TestReadToEmptyBuffer1)
{
    ...
    EXPECT_EQ(PSCError(PSC_COMM_NOT_ENOUGH_INPUT_DATA), stream.getError());
...
TEST(InputStreamTest, TestSetError)
{
    ...
    PSCError error = PSC_COMM_NOT_ENOUGH_BUFFER_SIZE;
    stream.setError(error);

    EXPECT_EQ(error, stream.getError());

    stream.setError(PSC_COMM_NOT_ENOUGH_INPUT_DATA);
    EXPECT_EQ(error, stream.getError());
}
