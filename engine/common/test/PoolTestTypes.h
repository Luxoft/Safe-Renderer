#ifndef POPULUSSC_POOLTESTTYPES_H
#define POPULUSSC_POOLTESTTYPES_H
...
public:
    ...
    PSCError allocateFailed()
    {
        PSCError error = PSC_NO_ERROR;
        const void* ptr = this->m_pool->allocate(error);
        EXPECT_TRUE(error != PSCError(PSC_NO_ERROR));
        EXPECT_TRUE(NULL == ptr);
        return error;
    }
...
#endif // POPULUSSC_POOLTESTTYPES_H
