...
public:
    ...
    typename T::ObjectType* createObject(PSCError& error)
    {
        return m_factory->createObject(error);
    }

    PSCError deleteObject(typename T::ObjectType* object)
    {
...
void Defragmentation(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_NO_ERROR), error);

    EXPECT_EQ(PSCError(PSC_NO_ERROR), factory->deleteObject(objectFirst));
...
void DeleteObject(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_NO_ERROR), factory->deleteObject(object));
}
...
void DeleteObjectTwice(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_NO_ERROR), factory->deleteObject(object));
    EXPECT_EQ(PSCError(PSC_POOL_DOUBLE_DELETE), factory->deleteObject(object));
}
...
void DeleteObjectWithNullPointer(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_POOL_INVALID_OBJECT), factory->deleteObject(NULL));
}
...
void DeleteObjectWithPointerFromAnotherBuffer(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_POOL_INVALID_OBJECT), factory->deleteObject(object - 0x50U));

    // Pointer to memory after storage
    EXPECT_EQ(PSCError(PSC_POOL_INVALID_OBJECT), factory->deleteObject(object + 0x7FFFU));
}
...
void DeleteObjectWithShiftedPointer(PooledObjectFactoryTest<T>* factory)
{
    ...
    EXPECT_EQ(PSCError(PSC_POOL_INVALID_OBJECT), factory->deleteObject(object));
...
TYPED_TEST(PooledObjectFactoryTest, CreateTooManyObjects)
{
    ...
    typename TypeParam::ObjectType* object = this->m_factory->createObject(error);
    EXPECT_EQ(PSCError(PSC_POOL_IS_FULL), error);
