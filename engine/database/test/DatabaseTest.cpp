...
TEST_F(BitmapAccessTest, BitmapAccess)
{
    ...
    const DDHType* ddh = static_cast<const DDHType*>(m_ddhbin.getData());
    {
        ...
        EXPECT_EQ(PSC_DB_IMGBIN_VERSION_MISMATCH, access.getError());
    }
    {
    ...
    }
    {
        ...
        EXPECT_EQ(PSC_NO_ERROR, access.getError());
    }
}
...
TEST_F(DatabaseTest, Database)
{
    // empty ddhbin
    {
        ...
        EXPECT_EQ(PSCError(PSC_DB_DDHBIN_VERSION_MISMATCH), db.getError());
    }
    // invalid ddhbin
    {
        U8 buf[] = { 1, 4, 55 };
    ...
    // empty imgbin
    {
        ...
        EXPECT_EQ(PSCError(PSC_DB_IMGBIN_VERSION_MISMATCH), db.getError());
    }
    // invalid imgbin
    {
        U8 buf[] = { 1, 4, 55 };
...
TEST_F(DatabaseTest, getBitmap)
{
    ...
    EXPECT_EQ(PSCError(PSC_NO_ERROR), db.getError());
    ResourceBuffer buf = db.getBitmap(1).getData();
    EXPECT_EQ(27800u, buf.getSize());
    PopulusImage image(buf);
    EXPECT_EQ(102u, image.getWidth());
    EXPECT_EQ(68u, image.getHeight());
    EXPECT_EQ(PopulusImage::PIXEL_FORMAT_BGRA8888, image.getPixelFormat());
