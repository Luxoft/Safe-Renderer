...
public:
    FonBinWriter(U8* buffer, U32 size)
        : buffer(buffer)
        , size(size)
        , header(reinterpret_cast<lsr::impl::FonBinHeader_t*>(buffer))
    ...
    U16 AddImage(const std::string& str, U16 width, U16 height)
    {
        if (0 == imageDataTable)
        {
            ...
            imageDataTable = reinterpret_cast<lsr::impl::ImageDataTable_t*>(buffer + header->imageDataTableOffset);
        }
    ...
    U8* buffer;
    U32 size;
    lsr::impl::FonBinHeader_t* header;
    U32 binaryDataOffset;
    lsr::impl::ImageDataTable_t* imageDataTable;
    U16 imageDataIx;
    U16 fontIx;
    U16 sceneIx;
};
