...
{
    LSRError transmitMessage(const U8* data, U32 dataLen) P_OVERRIDE {
        return LSR_NO_ERROR;
    }
};
...
TEST_F(DataHandlerTest, GetNumber)
{
    ...
    EXPECT_EQ(LSR_NO_ERROR, dataHandler.onMessage(&transmitter, MessageTypes::ODI, stream));
...
TEST_F(DataHandlerTest, onMessage)
{
    ...
    }
    {
        ...
        InputStream stream(buf, sizeof(buf));
        Number value;
        EXPECT_EQ(LSR_DH_INVALID_MESSAGE_TYPE, dataHandler.onMessage(&transmitter, MessageTypes::ODI, stream));
    }
    {
        msg.setFuId(53);
        msg.setInvalidFlag(false);
